...
The exponential model S(t) = exp(-lambdat) provides a simple way to model survival, but finding the right lambda value is crucial for a good fit to the data!
...
for(lam in test_lambdas) {
  # Log-likelihood formula: d * log(lambda) - lambda * sumt_i
...
cat("=== Search Results ===\n")
cat("Best lambda from search:", best_lambda, "\n")
cat("Log-likelihood at best lambda:", round(best_ll, 2), "\n")
...
for(lam in visual_lambdas) {
  ll <- d * log(lam) - lam * total_time
  diff <- best_ll - ll
  cat("lambda =", lam, ": Log-likelihood =", round(ll, 2), 
...
Let's visualize how the likelihood changes across different lambda values:
...
ggplot(results, aes(x = lambda, y = log_likelihood)) +
  ...
  labs(
    title = "Log-Likelihood Function for Different lambda Values",
    subtitle = paste("Red triangle = Optimal lambda (", best_lambda, "), Gray lines = Visual guesses", sep=""),
    x = "Lambda (lambda)",
    y = "Log-Likelihood"
  ) +
  theme_minimal() +
...
cat("Our visual 'good fit' was lambda = 0.9\n")
cat("Mathematical optimum is lambda =", best_lambda, "\n")
...
```{r analytical-mle}
# The MLE formula: lambda_hat = d / sumt_i
...
cat("lambda_MLE = d / sumt_i = ", d, " / ", round(total_time, 2), " = ", round(lambda_mle_analytical, 4), "\n", sep="")
cat("Log-likelihood:", round(ll_analytical, 2), "\n")

cat("\n=== Comparison of Methods ===\n")
cat("Grid search best lambda:", best_lambda, "\n")
cat("Analytical MLE lambda:", round(lambda_mle_analytical, 4), "\n")
...
final_labels <- c("lambda = 0.9 (Visual guess)", paste("lambda =", best_lambda, "(MLE)"))
...
mle_label <- paste("lambda =", best_lambda, "(MLE)")
color_mapping <- c("lambda = 0.9 (Visual guess)" = "green")
...
cat("\n=== Conclusion ===\n")
cat("• Visual assessment (lambda = 0.9) was very close to optimal!\n")
cat("• Mathematical MLE gives lambda =", round(lambda_mle_analytical, 3), "\n")
